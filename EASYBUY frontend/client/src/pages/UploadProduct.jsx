...
const UploadProduct = () => {
  ...
  return (
    <section className='min-h-full'>
      {/* Top Header Card */}
      <div className='sticky top-24 lg:top-20 z-10 bg-white/95 backdrop-blur-md rounded-2xl shadow-sm border border-gray-100 p-6 mb-6 flex items-center justify-between'>
        <h2 className='font-bold text-xl lg:text-2xl text-gray-800'>Upload Product</h2>
      </div>
        ...
        <form className='grid gap-8' onSubmit={handleSubmit}>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            <div className='grid gap-2'>
              <label htmlFor='name' className='font-bold text-gray-700 ml-1'>Product Name</label>
              <input
                id='name'
                type='text'
            ...
            <div className='grid gap-2'>
              <label htmlFor='unit' className='font-bold text-gray-700 ml-1'>Unit / Weight</label>
              <input
                id='unit'
                type='text'
          ...
          <div>
            ...
            <div className='flex flex-wrap gap-4'>
              <label htmlFor='productImage' className='w-32 h-32 bg-gray-50 border-2 border-dashed border-gray-200 rounded-3xl flex justify-center items-center cursor-pointer hover:border-[#00b050] hover:bg-[#00b050]/5 transition-all group'>
              ...
              <div className='flex flex-wrap gap-2 mt-2'>
                {data.category.map((c, index) => (
                  <div key={c._id + index} className='flex items-center gap-2 bg-[#00b050]/10 text-[#00b050] font-bold text-xs px-3 py-1.5 rounded-xl border border-[#00b050]/20'>
                    <p>{c.name}</p>
                    <button type='button' className='hover:text-red-500' onClick={() => handleRemoveCategory(index)}>
                      <IoClose size={16} />
                    </button>
                  </div>
                ))}
            ...
            <div className='grid gap-2'>
              ...
              <div className='flex flex-wrap gap-2 mt-2'>
                {data.subCategory.map((c, index) => (
                  <div key={c._id + index} className='flex items-center gap-2 bg-[#00b050]/10 text-[#00b050] font-bold text-xs px-3 py-1.5 rounded-xl border border-[#00b050]/20'>
                    <p>{c.name}</p>
                    <button type='button' className='hover:text-red-500' onClick={() => handleRemoveSubCategory(index)}>
                      <IoClose size={16} />
                    </button>
                  </div>
                ))}
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            <div className='grid gap-2'>
              <label htmlFor='stock' className='font-bold text-gray-700 ml-1'>Stock Quantity</label>
              <input
            ...
            <div className='grid gap-2'>
              <label htmlFor='price' className='font-bold text-gray-700 ml-1'>Base Price (₹)</label>
              <input
            ...
            <div className='grid gap-2'>
              <label htmlFor='discount' className='font-bold text-gray-700 ml-1'>Discount (%)</label>
              <input
          ...
          <div className='space-y-6'>
            {Object.keys(data.more_details).map((k, index) => (
              <div key={k + index} className='grid gap-2'>
                <label htmlFor={k} className='font-bold text-gray-700 ml-1'>{k}</label>
                <input
