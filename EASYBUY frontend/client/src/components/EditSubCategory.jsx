...
const EditSubCategory = ({ close, data, fetchData }) => {
    ...
    return (
        <section className='fixed top-0 right-0 bottom-0 left-0  bg-black/60 z-50 animate-fadeIn flex items-center justify-center p-4 backdrop-blur-sm'>
            <div className='w-full max-w-2xl bg-white p-8 rounded-3xl shadow-2xl relative animate-in fade-in zoom-in duration-300'>
                ...
                <form className='grid gap-6' onSubmit={handleSubmitSubCategory}>
                    <div className='grid gap-2'>
                        <label htmlFor='name' className='text-sm font-semibold text-gray-700'>Sub Category Name</label>
                        <input
                            id='name'
                            name='name'
                            ...
                            className='p-4 bg-gray-50 border border-gray-200 outline-none focus:border-green-500 focus:ring-4 focus:ring-green-100 rounded-2xl transition-all'
                        />
                    </div>
                    ...
                    <div className='grid gap-2'>
                        ...
                        <div className='flex flex-col lg:flex-row items-center gap-6'>
                            <div className='border-2 border-dashed border-gray-200 h-40 w-full lg:w-40 bg-gray-50 rounded-2xl flex items-center justify-center overflow-hidden group relative'>
                                {
                            ...
                            <label htmlFor='uploadSubCategoryImage' className='cursor-pointer w-full lg:w-auto'>
                                <div className='px-6 py-3 border-2 border-green-600 text-green-600 font-bold rounded-2xl hover:bg-green-600 hover:text-white transition-all text-center'>
                                    Change Image
                                </div>
                                <input
                                    type='file'
                            ...
                            <div className='flex flex-wrap gap-2 min-h-[40px]'>
                                {
                                    subCategoryData.category.map((cat) => (
                                        <div key={cat._id + "selectedValue"} className='bg-green-50 text-green-700 border border-green-100 px-3 py-1.5 rounded-xl flex items-center gap-2 animate-in slide-in-from-left-2 duration-200'>
